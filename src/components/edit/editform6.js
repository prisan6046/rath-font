...
class ShowFormSix extends Component{
        ...
        fetch(url+'/get_doc_four_id?id=' + this.props.id + "&token=" + this.state.token)
            ...
            .then((res) => {
               this.setState({
                ...
                date_out : res['0']['date_out'],
                location_out: res['0']['location_out'],
                staff_out: res['0']['staff_out'],
    ...
    render() {
        ...
        this.state.data_user.map((val, i) => {
            if(val._id.$oid == this.state.staff_id){
                            ...
                            <div className="card">
                                ...
                                <div className="card-body">
                                    <div className="row mb-2">
                                        ...
                                        <div className="col-lg-8">
                                            <input type="radio" name="type_check" id="1" value="ควันดำ"  checked={this.state.type_check === "ควันดำ"} defaultChecked={this.state.type_check === 'ควันดำ'} onChange={this.handleTypeCheckChange} /> <label className="mb-0" htmlFor="1">ควันดำ</label> <br/>
                                            <input type="radio" name="type_check" id="2" value="เสียงดัง" checked={this.state.type_check === "เสียงดัง"} defaultChecked={this.state.type_check === 'เสียงดัง'} onChange={this.handleTypeCheckChange} /> <label className="mb-0" htmlFor="2">เสียงดัง</label> <br/>
                                            <input type="radio" name="type_check" id="3" value="ไฮโดรคาร์บอน" checked={this.state.type_check === "ไฮโดรคาร์บอน"} defaultChecked={this.state.type_check === 'ไฮโดรคาร์บอน'} onChange={this.handleTypeCheckChange} /> <label className="mb-0" htmlFor="3">ไฮโดรคาร์บอน</label> <br/>
                                            <input type="radio" name="type_check" id="4" value="คาร์บอนไดออกไซด์" checked={this.state.type_check === "คาร์บอนไดออกไซด์"} defaultChecked={this.state.type_check === 'คาร์บอนไดออกไซด์'} onChange={this.handleTypeCheckChange} /> <label className="mb-0" htmlFor="4">คาร์บอนไดออกไซด์</label>
                                        </div>
                                    </div>
    ...
    // componentDidMount() {
    //     console.log("id 6 : "+this.props.id)
    //     this.state.token = localStorage.getItem('token');
    //     fetch(url+'/get_all_user?token=' + this.state.token)
    //         .then((Response) => Response.json())
    //         .then((res) => {
